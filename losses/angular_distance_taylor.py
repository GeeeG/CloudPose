...
def exponential_map(axag, EPS=1e-2):
    ...
    # term_1 = 1 - (theta_sq / 6) + (theta_pow_4 / 120) - (theta_pow_6 / 5040) + (theta_pow_8 / 362880)
    # term_2 = 0.5 - (theta_sq / 24) + (theta_pow_4 / 720) - (theta_pow_6 / 40320) + (theta_pow_8 / 3628800)
    # Rodrigues' formula for other angles
    # term_1 = tf.sin(theta) / theta
    # term_2 = (1 - tf.cos(theta)) / theta_sq
...
def logarithm(R, EPS=1e-2):
    ...
    # is_trace_one = tf.greater(trace_temp, 0.9999999)
    # is_trace_minus_one = tf.less(trace_temp, -0.9999999)
    #
    # safe_trace_one = tf.where(is_trace_one, tf.zeros(R.shape[0], dtype=tf.float64), trace_temp)
    # safe_trace = tf.where(is_trace_minus_one, tf.zeros(R.shape[0], dtype=tf.float64), safe_trace_one)
    #
    # safe_theta = tf.acos(safe_trace)
    # theta_one = tf.where(is_trace_one,
    #                      tf.zeros(R.shape[0], dtype=tf.float64),
    #                      safe_theta) # acos(1) = 0
    # theta = tf.where(is_trace_minus_one,
    #                  tf.cast(tf.fill([R.shape[0]], math.pi),dtype=tf.float64),
    #                  theta_one) # acos(-1) = pi
